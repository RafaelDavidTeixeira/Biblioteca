...
def charts_page():
    inst = db.get_institution()
    return render_template('app.html', page='charts', institution=inst,
                           user_name=session.get('user_name'), user_role=session.get('user_role'))
...
def dashboard_charts():
    return jsonify(db.dashboard_charts())
...
def get_books():
    ...
    return jsonify(db.list_books(q, page, per_page, sort_by, sort_order))


@bp.route('/api/books/search', methods=['GET'])
@auth_required
def search_books():
    """Busca rápida sem paginação (para modal de empréstimos)"""
    q = request.args.get('q', '')
    limit = request.args.get('limit', 50, type=int)
    sort_by = request.args.get('sort_by', 'title')
    sort_order = request.args.get('sort_order', 'asc')
    return jsonify(db.search_books(q, limit, sort_by, sort_order))
